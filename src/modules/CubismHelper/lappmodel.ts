...
export class LAppModel extends CubismUserModel {
  ...
  public update(): void {
    ...
    // リップシンクの設定
    if (this._lipsync) {
      // const value = 0.0; // リアルタイムでリップシンクを行う場合、システムから音量を取得して、0~1の範囲で値を入力します。

      for (let i = 0; i < this._lipSyncIds.getSize(); ++i) {
        this._model.addParameterValueById(this._lipSyncIds.at(i), this._volume, 0.8);
      }
    }
